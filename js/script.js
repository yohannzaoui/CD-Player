/**
 * Technics SL-PS740A - Core Logic
 * Author: Yohann Zaoui
 * Copyright: © 2026 Yohann Zaoui
 */
...
// Colors matching the new CSS VFD theme
const COLOR_CYAN = "#00ffcc";
const COLOR_RED = "#ff3c22";
const COLOR_ORANGE = "#ff9d00";
    ...
    if (window.jsmediatags) {
        window.jsmediatags.read(file, {
            ...
            onError: function() { 
                ...
                modalImg.src = "img/Technics_logo.png"; 
            }
        });
    }
    updateTrackDisplay(); 
...
function updateMediaSession(t, a, al) {
    if ('mediaSession' in navigator) {
        navigator.mediaSession.metadata = new MediaMetadata({ 
            ...
            artwork: [{ src: 'img/favicon_512.png', sizes: '512x512', type: 'image/png' }] 
        });
...
function drawVU() {
    ...
    // Smooth peak fall
    if (currentVol < lastVolume) lastVolume -= 1.8; else lastVolume = currentVol;
    ...
    for(let ch = 0; ch < 2; ch++) {
        ...
        let level = (ch === 0) ? lastVolume : lastVolume * (0.96 + Math.random()*0.08);
        
        for(let i = 0; i < segments; i++) {
            let threshold = (i / segments) * 255;
            if (level > threshold && level > 10) { 
                // Color threshold logic matching the VFD aesthetic
                ctx.fillStyle = (i > 18) ? COLOR_RED : (i > 15 ? COLOR_ORANGE : COLOR_CYAN); 
            } else { 
                ctx.fillStyle = "#111"; // Off segment
            }
...
audio.onended = () => { 
    ...
    else if(repeatMode === 2 || currentIndex < playlist.length - 1) { nextTrack(); } 
    else { handleStop(); } 
};
...
// --- Jog Shuttle Logic ---
function doShuttle(v) { 
    if(v != 0) { 
        audio.currentTime += (v * 0.4); 
        statusFunc.innerText = v > 0 ? "SEARCH >>" : "<< SEARCH"; 
    } 
}

function resetShuttle() { 
    document.getElementById('shuttle').value = 0; 
    if (audio.src) statusFunc.innerText = audio.paused ? "PAUSE" : "PLAY"; 
}
...
function runAutoCue() {
    if (playlist.length === 0) return;
    statusFunc.innerText = "AUTO CUE"; 
    audio.pause(); 
    audio.currentTime = 0;
    setTimeout(() => { 
        audio.currentTime = 0.1; 
        statusFunc.innerText = "A.CUE READY"; 
    }, 800);
}
...
function runPeak() {
    ...
    let peakTime = audio.duration * 0.75; // Simulated peak
    setTimeout(() => {
